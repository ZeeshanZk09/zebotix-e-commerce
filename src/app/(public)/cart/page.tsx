...
import { deleteItemFromCart, fetchCart } from '@/lib/redux/features/cart/cartSlice';
...
export default function Cart() {
  ...
  const cartItemsLoading = useAppSelector((state) => state.cart.isLoading);
  ...
  if (productsLoading && cartItemsLoading && (!cartArray || cartArray.length === 0))
    return <Loading />;
    ...
    <>
                ...
                <table className='w-full max-w-4xl text-slate-600 table-auto'>
                  <thead>
                    <tr className='max-sm:text-sm'>
                      ...
                      <th>Quantity</th>
                      <th className='min-w-[5rem]'>Total Price</th>
      ...
      <SignedOut>
        <div className='min-h-screen flex justify-center items-center'>
          <SignIn routing='hash' fallbackRedirectUrl={'/cart'} />
        </div>
      </SignedOut>
    </>
  );
