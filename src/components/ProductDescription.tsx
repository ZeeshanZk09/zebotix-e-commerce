...
const ProductDescription = ({ product }: { product: ProductCreateInput }) => {
  ...
  return (
    <div className='my-18 text-sm text-slate-600'>
      ...
      <div className='flex gap-3 mt-14'>
        <Image
          src={product.store.connect?.logo as string}
          alt=''
        ...
        <div>
          <p className='font-medium text-slate-600'>
            Product by {product.store.connect?.name as string}
          </p>
          <Link
            href={`/shop/${product.store.connect?.username as string}`}
