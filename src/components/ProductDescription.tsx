...
const ProductDescription = ({ productId }: { productId: string }) => {
  ...
  const product: any = useAppSelector((state) => state.product.list).find(
    (p) => p.id === productId
  );
      ...
      <div className='my-18 text-sm text-slate-600'>
        ...
        <div className='flex border-b border-slate-200 mb-6 max-w-2xl'>
          {['Description', 'Reviews'].map((tab, index) => (
            <button
              className={`${
                tab === selectedTab ? 'border-b-[1.5px] font-semibold' : 'text-slate-400'
        ...
        {selectedTab === 'Reviews' && (
          <div className='flex flex-col gap-3 mt-14'>
            {Array.isArray(product.rating) &&
              product.rating.map((item: any, index: any) => (
        ...
        <div className='flex gap-3 mt-14'>
          ...
          />
          <div>
            <p className='font-medium text-slate-600'>Product by {product.store?.name as string}</p>
            <Link
              href={`/shop/${product.store?.username as string}`}
